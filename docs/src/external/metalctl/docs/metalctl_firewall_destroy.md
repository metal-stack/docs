...
destroy a firewall and destroy all data stored on the local disks. Once destroyed it is back for usage by other projects.
A destroyed firewall can not restored anymore

```
metalctl firewall destroy <firewall ID> [flags]
```
