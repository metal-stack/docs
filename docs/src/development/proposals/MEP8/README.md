...
- [ ] Clarify compatibility with `reinstall` feature
- [ ] Migrate existing allocations to use the default fsl, check whether the s2-large are migrated correct.
- [ ] For the s2-large machines we actually figure out which of the /dev/sd* disk has less than 1TB in size and install on this device. This type of flexibility is gone with this approach.
